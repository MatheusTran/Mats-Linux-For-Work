	...
	--arturl)
		...
		if [ -z "$url" ]; then
			...
			if [[ "$url" == file://* ]]; then
				url=${url#file://}
				# old_url=${url#file://}
				# file_name=$(echo $old_url | awk -F '/' '{print $7}')
                # file_extension=$(echo $file_name | awk -F '.' '{print $2}')
                # url="$HOME/.config/hypr/hypr_lock_thumbnail.$file_extension"
                # cp $old_url $url
			elif [[ "$url" == https://i.scdn.co/* ]]; then
				cd "$HOME/.config/hypr/thumbnails"
				...
				url="$HOME/.config/hypr/thumbnails/${temp_url}"
			fi
            magick "$url" -gravity center -resize 512x512^ -extent 512x512 "/home/Mat/.config/hypr/thumbnails/hyprlock_thumbnail.webp"
			echo "/home/Mat/.config/hypr/thumbnails/hyprlock_thumbnail.webp"
		fi
		;;
	--artist)
