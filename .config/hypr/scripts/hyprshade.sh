...
if [[ "$1" == "rofi" ]]; then
    ...
    if [ ! -z $choice ]; then
        echo "hyprshade_filter=\"$choice\"" >~/.config/assets/settings/hyprshade.sh
        if [ "$choice" == "off" ]; then
            hyprshade off
    ...
    if [ -f ~/.config/assets/settings/hyprshade.sh ]; then
        source ~/.config/assets/settings/hyprshade.sh
    fi
